...
/**
 * Controlador REST para gestión de Personal CNT
 */
@RestController
...
public class PersonalCntController {
    ...
    public ResponseEntity<PersonalResponse> uploadFoto(@PathVariable Long id,
        ...
        log.info("Subiendo foto para personal CNT con ID {}", id);
    ...
    public ResponseEntity<Void> deleteFoto(@PathVariable Long id) throws IOException {
        log.info("Eliminando foto de personal CNT con ID {}", id);
    ...
    public ResponseEntity<Resource> getFoto(@PathVariable Long id) {
        log.info("Obteniendo foto de personal CNT con ID {}", id);
        ...
        try {
            ...
            Path filePath = Paths.get(uploadDir, personal.getFoto());
            ...
            log.error("Error al leer el archivo de la foto: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
    ...
    /**
     * Asegura que el tipo de personal sea siempre CENATE
     */
