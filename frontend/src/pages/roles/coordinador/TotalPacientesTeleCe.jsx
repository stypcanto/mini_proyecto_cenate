/**
 * TotalPacientesTeleCe.jsx
 * Vista "Total Pacientes TeleCe" — distribución de pacientes por médico (Medicina Especializada SGDT)
 * v1.85.36 — Misma lógica que TotalPacientesEnfermeria pero para SGDT/Medicina Especializada
 *
 * Acceso: COORD. ESPECIALIDADES / SUPERADMIN
 * Datos: GET /api/sgdt/estadisticas/por-medico?fecha=YYYY-MM-DD&turno=MANANA|TARDE
 *        GET /api/sgdt/pacientes/por-medico?idPersonal=&fecha=YYYY-MM-DD
          ...
          {filtrados.map(m => (
            <div key={m.idPersonal}
              ...
              style={{
                ...
                color: String(m.idPersonal) === String(value) ? '#0D5BA9' : '#1e293b',
...
// ══════════════════════════════════════════════════════════════
// Drawer lateral — detalle de pacientes de un médico
// ══════════════════════════════════════════════════════════════
function DrawerMedico({ medico, fecha, turno, onClose, onReasignacionExitosa }) {
  ...
  useEffect(() => {
    ...
    const cargarHoras = async () => {
      ...
      try {
        const data = await apiClient.get(`/sgdt/pacientes/por-medico?idPersonal=${medicoDestino}&fecha=${fechaReasig}`, true);
        const lista = Array.isArray(data) ? data : [];
        const horas = lista.map(p => p.horaCita || p.hora_cita).filter(Boolean).map(h => h.substring(0, 5));
  ...
  useEffect(() => {
    const cargar = async () => {
      setLoading(true);
      try {
        let url = `/sgdt/pacientes/por-medico?idPersonal=${medico.id_medico}`;
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        const data = await apiClient.get('/sgdt/medicos', true);
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        const data = await apiClient.get(`/sgdt/estadisticas/fechas-por-medico?idPersonal=${medico.id_medico}`, true);
        setDiasConDatosMedico(data && typeof data === 'object' ? data : {});
      } catch { setDiasConDatosMedico({}); }
    };
    cargar();
  }, [medico.id_medico]);
  ...
  const confirmarReasignacion = async () => {
    ...
    try {
      await apiClient.put('/sgdt/reasignar-masivo', { ids, idPersonal: Number(medicoDestino), fecha: fechaReasig, hora: horaReasig }, true);
  ...
  const exportarExcel = () => {
    ...
    XLSX.writeFile(wb, `Pacientes_${medNombre}${fechaLabel}${turnoLabel}${estadoLabel}.xlsx`);
  };
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                ...
                <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: '11px', marginTop: '2px' }}>
                  {n(medico.total).toLocaleString()} pacientes asignados{fechaLocal && ` · ${fechaLocal}`}
                </div>
              </div>
            </div>
...
// ══════════════════════════════════════════════════════════════
export default function TotalPacientesTeleCe() {
  const [medicos,       setMedicos]       = useState([]);
  const [loading,       setLoading]       = useState(true);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const data = await apiClient.get(`/sgdt/pacientes/buscar-global?q=${encodeURIComponent(busqPaciente.trim())}`, true);
  ...
  const abrirMedicoDeResultado = (resultado) => {
    ...
    if (medico) {
      setMedicoDrawer(medico);
    } else {
      setMedicoDrawer({ id_medico: resultado.idPersonal, nombre_medico: resultado.nombreEnfermera, total: 0, pendientes: 0, atendidos: 0, desercion: 0 });
    }
  ...
  const cargar = useCallback(async (f, t) => {
    ...
    try {
      ...
      const url = '/sgdt/estadisticas/por-medico' + (qs ? `?${qs}` : '');
      const data = await apiClient.get(url, true);
      setMedicos(Array.isArray(data) ? data : []);
    } catch (err) {
  ...
  useEffect(() => {
    cargar(null, null);
    apiClient.get('/sgdt/estadisticas/fechas-disponibles', true)
    ...
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#0f172a' }}>Total Pacientes TeleCe</h2>
            <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
              SGDT · Medicina Especializada ·{' '}
                    ...
                    return (
                      <div key={r.idSolicitud || i} onMouseDown={() => abrirMedicoDeResultado(r)}
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                          <div style={{ width: '36px', height: '36px', borderRadius: '50%', flexShrink: 0, background: '#dbeafe', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: '800', color: '#1d4ed8' }}>
      ...
      <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '12px 16px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <div style={{ borderLeft: '1px solid #e2e8f0', paddingLeft: '16px', fontSize: '11px', color: '#64748b' }}>
            Datos de <strong>SGDT · Medicina Especializada</strong> · Filtro por fecha_atencion
          </div>
        </div>
      </div>
