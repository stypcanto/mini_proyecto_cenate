...
const ForgotPassword = () => {
    ...
    const handleForgot = async (e) => {
        ...
        try {
            ...
            setMessage(data?.message || "Se ha enviado un enlace de recuperación si el correo es válido.");
        } catch (err) {
            console.error(err);
                ...
                <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-10 text-center">
                    ...
                    <form onSubmit={handleForgot} className="space-y-5 text-left">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-4 px-6 bg-gradient-to-r from-[#2e63a6] to-[#1d4f8a] text-white font-bold rounded-xl shadow-lg hover:scale-[1.02] transition-transform flex justify-center items-center gap-2"
                        >
                            {loading ? (
                                <>
