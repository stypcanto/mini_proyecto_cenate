/**
 * BolsaXMedico.jsx
 * Vista "Bolsa x Médico" — distribución de pacientes por médico asignado
 * v1.0 – Estructura similar a BolsaXGestor con filtro por calendario
 *
 * Datos: GET /api/bolsas/solicitudes/estadisticas/por-medico?fechaDesde=&fechaHasta=
    ...
    <div style={{ background: '#fff', border: '1.5px solid #0D5BA9', borderRadius: '12px', overflow: 'hidden', minWidth: '280px', boxShadow: '0 4px 16px rgba(13,91,169,0.12)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', padding: '4px 8px 10px', gap: '2px' }}>
        ...
        {Array.from({ length: diasEnMes }, (_, idx) => {
          ...
          return (
            <button key={fechaStr} onClick={() => onSelectFecha(activo ? null : fechaStr)}
              ...
              {conteo > 0 && (
                <span style={{ marginTop: '2px', minWidth: '18px', height: '18px', borderRadius: '50%', fontSize: '9px', fontWeight: '800', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1, padding: '0 3px', background: activo ? 'rgba(255,255,255,0.3)' : '#0D5BA9', color: '#fff' }}>
                  {conteo > 9 ? '9+' : conteo}
                </span>
              )}
            </button>
      ...
      {fechaSeleccionada && (
        <div style={{ padding: '0 8px 10px', textAlign: 'center' }}>
          <button onClick={() => onSelectFecha(null)}
...
// ── Barra segmentada ──────────────────────────────────────────
function BarraSegmentada({ medico }) {
  const total = n(medico.total);
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      <div style={{ flex: 1, height: '10px', background: '#f1f5f9', borderRadius: '6px', overflow: 'hidden', display: 'flex', minWidth: '80px' }}>
        {CATS.map((c, i) => {
          const val = n(medico[c.key]);
          if (!val) return null;
          return <div key={i} title={`${c.label}: ${val} (${pct(val, total)}%)`} style={{ width: `${pct(val, total)}%`, background: c.accent, transition: 'width 0.5s' }} />;
        })}
      </div>
      <span style={{ fontSize: '11px', fontWeight: '700', color: '#10b981', minWidth: '32px', textAlign: 'right' }}>
        {pct(n(medico.atendidos), total)}%
      </span>
    </div>
  );
}
...
export default function BolsaXMedico() {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (calendarioRef.current && !calendarioRef.current.contains(e.target)) setShowCalendario(false);
    };
  ...
  const cargar = useCallback(async (fecha) => {
    ...
    setError('');
    try {
      let url = '/bolsas/solicitudes/estadisticas/por-medico';
      if (fecha) url += `?fechaDesde=${fecha}&fechaHasta=${fecha}`;
      const data = await apiClient.get(url, true);
      setMedicos(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error('Error cargando estadísticas por médico:', err);
  ...
  const totales = useMemo(() => {
    ...
    medicos.forEach(m => {
      t.total += n(m.total);
      CATS.forEach(c => { t[c.key] += n(m[c.key]); });
    });
    return t;
  }, [medicos]);
  ...
  if (loading) return (
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {Array.from({ length: 4 }, (_, i) => (
        <div key={i} style={{ height: i === 0 ? '32px' : i === 1 ? '52px' : '80px', background: '#f1f5f9', borderRadius: '12px', animation: 'pulse 1.5s infinite' }} />
      ))}
    ...
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#0f172a' }}>Bolsa x Médico</h2>
            <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
              {etiquetaPeriodo} · <strong style={{ color: '#0D5BA9' }}>{totales.total.toLocaleString()}</strong> pacientes · {medicos.length} médicos
            </p>
          </div>
        </div>
        <button onClick={() => cargar(fechaSeleccionada)}
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <div ref={calendarioRef} style={{ position: 'relative' }}>
            <button onClick={() => setShowCalendario(v => !v)}
              style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', border: `1.5px solid ${showCalendario || fechaSeleccionada ? '#0D5BA9' : '#cbd5e1'}`, background: showCalendario || fechaSeleccionada ? '#eff6ff' : '#f8fafc', color: fechaSeleccionada ? '#0D5BA9' : '#475569', fontSize: '13px', fontWeight: '600', transition: 'all 0.15s' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '10px' }}>
        <div style={{ background: '#0D5BA9', borderRadius: '12px', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '4px', boxShadow: '0 2px 8px rgba(13,91,169,0.25)' }}>
          <span style={{ fontSize: '11px', fontWeight: '600', color: 'rgba(255,255,255,0.75)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total Pacientes</span>
        ...
        {CATS.map(c => {
          ...
          return (
            <div key={c.key} onClick={() => setFiltroCat(prev => prev === c.key ? null : c.key)}
              title={`${activo ? 'Quitar filtro' : 'Filtrar'}: ${c.label}`}
              style={{ background: activo ? c.accent : c.bg, border: `2px solid ${activo ? c.accent : c.border}`, borderLeft: `4px solid ${c.accent}`, borderRadius: '12px', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '4px', cursor: 'pointer', transition: 'all 0.15s', boxShadow: activo ? `0 4px 12px ${c.accent}40` : '0 1px 2px rgba(0,0,0,0.04)', transform: activo ? 'translateY(-1px)' : 'none', userSelect: 'none' }}>
      ...
      <div style={{ position: 'relative', maxWidth: '320px' }}>
        ...
        <input type="text" placeholder="Buscar médico..." value={busquedaNombre}
        ...
        />
        {busquedaNombre && (
          <button onClick={() => setBusquedaNombre('')}
      ...
      {medicos.length === 0 ? (
        <div style={{ background: '#f8fafc', border: '1px dashed #cbd5e1', borderRadius: '12px', padding: '48px', textAlign: 'center', color: '#94a3b8' }}>
          <Stethoscope size={40} style={{ opacity: 0.2, marginBottom: '12px' }} />
                    ...
                    return (
                      <tr key={m.id_medico}
                        ...
                        <td style={{ padding: '11px 14px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: '#dcfce7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '700', color: '#16a34a', flexShrink: 0 }}>
                            ...
                            <div>
                              ...
                              <div style={{ fontSize: '10px', color: '#94a3b8', marginTop: '1px' }}>
                                {pct(n(m.citados) + n(m.atendidos), n(m.total))}% avanzado
                              </div>
                            </div>
                          </div>
                        </td>
                        ...
                        {CATS.map(c => (
                          <td key={c.key} style={{ padding: '11px 14px', textAlign: 'center' }}>
                            <Badge valor={n(m[c.key])} accent={c.accent} bg={c.bg} />
                          </td>
                        ))}
                        <td style={{ padding: '11px 14px' }}>
                          <BarraSegmentada medico={m} />
                        </td>
                      </tr>
                    );
                  })}
              ...
              <tfoot>
                <tr style={{ background: '#f1f5f9', borderTop: '2px solid #e2e8f0', fontWeight: '700' }}>
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <BarraSegmentada medico={totales} />
                  </td>
                </tr>
              </tfoot>
            </table>
      ...
      <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '12px 16px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', color: '#374151', borderLeft: '1px solid #e2e8f0', paddingLeft: '12px' }}>
            ...
            <span style={{ color: '#64748b' }}>Basado en fecha de asignación del paciente al médico</span>
          </div>
        </div>
      </div>
    </div>
...
const TH = {
  padding: '11px 14px', textAlign: 'center', fontSize: '10px',
  fontWeight: '700', color: '#e0f2fe', textTransform: 'uppercase', letterSpacing: '0.06em',
};
